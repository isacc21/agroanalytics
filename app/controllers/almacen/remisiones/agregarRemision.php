...
if(md5($_POST['pass'])==$_SESSION['password']){
	...
	if($remisionar == "listo"){
		...
		foreach($lista_pedidos as $row){
			...
			$mail->Body    = 'Se ha realizado una nueva remisión con folio: '.$folio.'. Esta notificación se envia para comenzar el procedimiento de facturación.';
			$mail->AltBody = 'Estoy haciendo pruebas para enviar información mediante correo electrónico';
