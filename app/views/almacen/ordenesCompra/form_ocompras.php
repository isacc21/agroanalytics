    ...
    <div class="portlet-body form">
      ...
      <form class="form-horizontal save-user" id="guardarCotizacion" >
        ...
        <div class="form-body">
          ...
          <div class="form-group">
          <label class="col-md-3 control-label">Agencia aduanal</label>
            <div class="col-md-7">
              <select id="agencia" class="form-control" required >
                <option selected disabled value="">Seleccione una agencia</option>
                <?php 
                $agencias=$ordenesCompra->consultarAgencias();
                foreach($agencias as $row){
                  $agencia = $row['idAgencia'];
                  $nombre = $row['comercialAgencia'];

                  ?>
                  <option value="<?=$agencia;?>"><? echo $nombre;?></option>
                  <?php } ?>
                </select>
              </div>
            </div>
