...
# 02 Abril 2017 : 12:25                                                              #
#                                                                                    #
###### cuentasCobrar/cat_cxcD.php ####################################################
#                                                                                    #
# Archivo sin estructura del catálogo de bancos para ser recibido por                #  
# JQuery en index de "Cuentas por Cobrar"                                            #
#                                                                                    #
###### HISTORIAL DE MODIFICACIONES ###################################################
#                                                                                    #
# 02-ABR-17: 12:26                                                                   #
# IJLM - Se copia CATALOGO de Bancos                                                 #
######################################################################################
...
$listaCXP = $usdCuentasP->consultarCuentasP();


###### CONSULTA DE ACREEDORES PARA VENTANAS MODALES ##################################
$consultaModal = $usdCuentasP->consultarCuentasP();
...
foreach ($result as $row){
  ...
  <div class="col-md-6">
  	<div class="btn-group">
  		<button id="goto_cxpUSD" class="btn sbold green"> 
  ...
  <ul class="dropdown-menu pull-right">
    ...
    <li>
      <a href="#" id="cxp_pend">
        <i class="fa fa-calendar-minus-o"></i> Solo pendientes a pago 
      </a>
    </li>
    <li>
      <a href="#" id="cxp_cancel">
        <i class="fa fa-times-circle"></i> Solo cancelados
      </a>
    </li>
    <li>
      <a href="#" id="cxp_sald">
        <i class="fa fa-check-circle"></i> Solo saldados
      </a>
    </li>
  </ul>';


  $html_saldado='<span class="label label-sm label-success"> Saldado </span>';
  $html_pendiente='<span class="label label-sm label-warning"> Pendiente a pago </span>';
    ...
    <div class="portlet light bordered">
     ...
     <div class="portlet-title">
      ...
      <div class="caption font-dark">
       ...
       <span class="caption-subject bold uppercase"> Listado de Cuentas</span>
     </div>
     <div class="actions">
      <div class="btn-group btn-group-devided" data-toggle="buttons">
        <?echo $html_filtros;?>
         <button type="button" name="back" id="back_cat_cp1" class="btn btn-secondary-outline">
...
<tbody>
  ...
  foreach($listaCXP as $row){
    ...
    if($acreedor!=""){
      ...
      $m_acreedor = $usdCuentasP->consultarAcreedoresxID();
      foreach($m_acreedor as $row){
        $v_acreedor = $row['razonSocAcreedor'];
      }
      echo '<td>'.$v_acreedor.'</td>';
    }
    if($proveedor!=""){
      $usdCuentasP->proveedor=$proveedor;
      $result_proveedor = $usdCuentasP->consultarProveedoresxID();
      foreach($result_proveedor as $row){
        $v_proveedor = $row['razonSocProveedor'];
        $razon = $row['rfcProveedor'];
      }
      echo '<td>'.$v_proveedor.'</td>';
    }
    ?>
    <td><?php echo '$ '.$num; ?></td>
    <td><?php
      ...
      <td>
        ...
        <button class="btn btn-xs green-seagreen dropdown-toggle" type="button" data-toggle="dropdown" aria-expanded="false"> Actions
          <i class="fa fa-angle-down"></i>
        </button><ul class="dropdown-menu pull-left" role="menu">';
        ...
        $html_saldar='<li><a><input type="radio" id="saldar'.$codigo.'" class="saldar" name="saldar" value="'.$codigo.'">
        <label for="saldar'.$codigo.'">  <i class="fa fa-check-square-o"></i>&nbsp;Saldar </label></a></li>';
  ...
  <div class="modal fade" id="modal<?=$codigo;?>" tabindex="-1" role="basic" aria-hidden="true">
    ...
    <div class="modal-dialog">
     ...
     <div class="modal-body">
       ...
       <tr>
         <td><?if($proveedor!=""){echo "Proveedor: ";}else{echo "Acreedor: ";}?> </td>
         <?php 
         if($proveedor!=""){
          $usdCuentasP->proveedor=$proveedor;
          $m_proveedor = $usdCuentasP->consultarProveedoresxID();
          foreach($m_proveedor as $row){
            $v_proveedor = $row['razonSocProveedor'];
          }
          echo '<td>'.$v_proveedor.'</td>';
        }
    ...
    <tr>
      <td><?php if($status==1){echo "Última edición por:";}else{if($status==2){echo "Cancelado por:";}else{if($status==3){echo "Saldado por:";}}} ?> </td>
...
<script>
    ...
    $("#back_cat_cp1").click(function(){
          window.location = ""
        });

    /* SCRIPT PARA CAMBIAR CONTENIDO POR FORMULARIO EN BLANCO */
    $("#cxp_all").click(function(){
      $("#mainContent").load( "cat_cxpD.php" );
    });

    /* SCRIPT PARA CAMBIAR CONTENIDO POR FORMULARIO EN BLANCO */
    $("#cxp_pend").click(function(){
      $("#mainContent").load( "cat_cxpD_pend.php" );
    });

    /* SCRIPT PARA CAMBIAR CONTENIDO POR FORMULARIO EN BLANCO */
    $("#cxp_cancel").click(function(){
      $("#mainContent").load( "cat_cxpD_cancel.php" );
    });

    /* SCRIPT PARA CAMBIAR CONTENIDO POR FORMULARIO EN BLANCO */
    $("#cxp_sald").click(function(){
      $("#mainContent").load( "cat_cxpD_sald.php" );
    });



    /* SCRIPT PARA CAMBIAR CONTENIDO POR FORMULARIO EN BLANCO */
    $("#goto_cxpUSD").click(function(){
    ...
    $('.editar').click(function() {
     $("#mainContent").load( "form_cxpD.php?codigo="+$(this).val());

   });

    /* SCRIPT PARA ENVIO DE FOLIO Y ELIMINACION DEL ACREEDOR EN CUESTION*/ 
    $('.cancelar').click(function() {
      $("#mainContent").load( "conf_usd_del.php?codigo="+$(this).val());

    });

    /* SCRIPT PARA ENVIO DE FOLIO Y ELIMINACION DEL ACREEDOR EN CUESTION*/ 
    $('.saldar').click(function() {
      $("#mainContent").load( "conf_usd_saldar.php?codigo="+$(this).val());
