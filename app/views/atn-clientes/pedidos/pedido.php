...
$pedidos->folio = $_REQUEST['codigo'];
$lista = $pedidos->consultarPedidosxID();
foreach($lista as $row){
	$rfc = $row['rfcCliente'];
	$fecha = $row['ddPedido']."/".$row['mmPedido']."/".$row['yyyyPedido'];
}

$pedidos->cliente = $rfc;
$result = $pedidos->consultarClientes();
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(99);
		$this->Cell(60,10,utf8_decode(' PEDIDO '),0,0,'C',1);
...
foreach($lista as $row){
	...
	$pedidos->folio = $_REQUEST['codigo'];
	$productos = $pedidos->consultarDetalle();
	...
	foreach($productos as $row){
		...
		$unidad_cotizacion = $row['unidadDetallePedido'];
		...
		$preciot = $row['montoDetallePedido'];
		$qty = $row['cantidadDetallePedido'];
		$supert += $preciot;

		$pedidos->producto = $row['codigoProducto'];
		$pf = $pedidos->consultarProductosxID();
		...
		$pdf->SetX(10);
		$pdf->SetFillColor(255,255,255);
		$pdf->Cell(50,6,utf8_decode($nombre_producto),0,0,'L',1);
		$pdf->Cell(25,6,$presentacion,0,0,'L',1);
		$pdf->Cell(21,6,$qty,0,0,'C',1);
		$pdf->Cell(20,6,$unidad,0,0,'C',1);
		$pdf->Cell(20,6,"$ ".$unitario."  ",0,0,'R',1);
		$pdf->Cell(28,6,"$ ".$total."  ",0,0,'R',1);
		$pdf->Cell(30,6,utf8_decode('DÓLAR USD'),0,0,'C',1);
		$pdf->Ln();
	}
